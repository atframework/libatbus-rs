...
impl FrameBlockAlgorithm {
    ...
    pub fn hash<U: AsRef<[u8]>>(rem: &U) -> [u8; FRAME_HASH_SIZE] {
        ...
        let mut cursor = io::Cursor::new(rem);
    ...
    #[inline]
    pub fn decode_varint<U: AsRef<[u8]>>(input: &U) -> ProtocolResult<VarintData> {
    ...
    #[inline]
    pub fn encode_varint<T: AsMut<[u8]>, U: Into<u64>>(
        output: &mut T,
        input: U,
...
mod test {
    ...
    fn test_decode_varint_error() {
        let decode_result = FrameBlockAlgorithm::decode_varint(&[0x96]);
